...
def gen_checklists_markup(user_id):
    ...
    markup = InlineKeyboardMarkup()
    markup.row_width = len(checklists) + 1
...
def gen_add_activities_to_checklist_markup(user_id, checklist_id):
    ...
    markup = InlineKeyboardMarkup()
    markup.row_width = len(activities) + 1
    for act_id, name, creation_date, owner, is_bool, measurement in activities:
        ...
        activity_name += name
        callback_data = f'activity_to_checklist_{act_id}_{checklist_id}'
        markup.add(
            InlineKeyboardButton(activity_name, callback_data=callback_data)
        )
    markup.add(InlineKeyboardButton('Подтвердить выбор', callback_data='accept_activites'))
    return markup
